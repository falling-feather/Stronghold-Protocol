...
- [x] v3.19.0 应用内 Modal/Toast 系统替换原生弹窗：新增 `src/core/ModalSystem.ts` 提供 `showToast(level/duration)` 与 `showAlert/showConfirm(title,body):Promise` 三个 API；CSS 追加 `.sp-toast-*` 顶部居中堆叠条与 `.sp-modal-*` 居中遮罩对话框（z-index 100000/100001，支持 info/success/warn/error 与 primary/danger/ghost 三种按钮风格）；GameEngine `tryStartCombat` 改 async（资金未花完用 modal 二次确认）、6 处 `alert` 拆分为 modal（胜利/失败/成就解锁）与 toast（波次完成/备战区满/瞬发技能未实现）；ShopSystem 6 处 alert 全改 toast；MetaScreen confirm/alert、FactionScreen / RosterScreen / PactScreen / MultiplayerScreen 各 1 处也一并替换，全工程不再有原生 `alert/confirm` 调用
- [x] v3.19.1 ModalSystem 易用性补强：新增 Esc 键 / 点击遮罩关闭 dialog（栈式管理，仅触发最顶层）；`DialogOpts` 增 `dismissValue` 字段，`showAlert` 默认 `undefined`、`showConfirm` 默认 `false`；可设 `'none'` 强制锁死（如失败/通关弹窗仍可被 Esc 关闭以触发 reload）
- [x] v3.20.0 全局 LayoutManager + UI 自适应：新增 `src/core/LayoutManager.ts`（`initLayoutManager / applyLayout / onLayoutChange / isRotatedLayout`），`main.ts` bootstrap 阶段一次性安装 `resize / orientationchange` 监听；旋转规则 = 视口竖屏且最短边 ≤ 900 且（UA 移动设备 或 宽 ≤ 540）→ 给 `<html>` 加 `mobile-rotated`（CSS `transform: rotate(90deg)` 整体旋转覆盖所有屏幕，不再仅 GameScreen）；同时按宽高比写入 `aspect-portrait/tall/square/wide/ultrawide` 之一供 CSS 微调；`GameScreen` 移除自有 `isMobileDevice` / `handleResize` 重复逻辑改订阅 `onLayoutChange`；`main.css` 追加 `.aspect-*` 与 `html.mobile-rotated` 下菜单/Modal/Toast 字号网格适配
- [x] v3.20.1 手机旋转模式全屏修复 + 局内盟约详情 + 最高星合并赠送：(1) `main.css` 中 `html.mobile-rotated` 下所有菜单屏（pact/roster/meta/boon/achievement/daily/mp/settings）改用 `position:absolute; inset:0; width/height:100%; overflow:hidden; flex column` 强制撑满旋转后视口（修复部分浏览器 `position:fixed` 不跟随 `transformed ancestor` 导致的内容只占左上角问题），并按屏分别收紧 header/grid 列宽/字号；(2) `GameScreen.renderPactBadges` 给 pact-badge 与 resonance-badge 增加 `cursor:pointer` 与 `data-pact-id/data-reso-id`，点击调用 `showAlert` 弹出详细面板（当前层数 / 已激活档位 / 下一档差距 / 完整档位表 / 叠层来源 / 衰减规则 / 影响范围 / 枷锁代价；共鸣同理列出激活条件与影响范围），手机端无 hover 也可查；(3) `ShopSystem.enterTemporaryShop` 当合并目标已是最高星（无 next shopLevel）时改为从同档（最高星）随机干员中开临时商店赠送一个免费名额，避免「无事发生」- [x] v3.20.2 战斗页 UI A 精修：(1) `index.html` 中 top-bar 资源文字 “生命/资金/时间/波次” 改为图标 `❤ 💰 ⏱ 🌊` + `title` 提示，缩减带宽；`#pact-badges` 改单行水平滚动 + 右侧 `mask-image` 渐变隐去，避免换行撞坏顶栏；(2) `#shop-area` 改为 `grid 36px / 1fr / 92px` 三列固定布局，「刷新/升级」始终可见；`#shop-cards` 从 flex 横滚改为 `grid auto-fill minmax(96px,1fr)` 自适应换行 + 竖向滚动；(3) `.shop-card` 高度调为 76px / 手机端 62px，`.bench-slot` 调为 64px / 手机端 50px，`#bottom-area.min-height` 220 → 200 / 手机端 160px，释放画布纵向空间；(4) 删除 `#detail-panel.visible ~ #canvas-wrapper { transform: translateX(140px) }`，detail 面板滑出时画布不再被挤出视野，改为半透明叠加
## v4.x — 联机
